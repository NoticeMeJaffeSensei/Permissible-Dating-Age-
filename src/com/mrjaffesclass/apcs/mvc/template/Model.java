...
public class Model implements MessageHandler {
  ...
  public void messageHandler(String messageName, Object messagePayload) {
    ...
    int yourAge = payload.getDirectionX();
    int loverAge = payload.getDirectionY();
    
    if (messageName.equals("view:changeButton")) {
     double a = yourAge/2 +7;
     double b = Math.round(a); //Lower limit
     double c = yourAge - b;
     double d = yourAge + c; //Upper limit
     mvcMessaging.notify("model:lowerLimit", b, true);
     mvcMessaging.notify("model:upperLimit", d, true);
     
     if (loverAge < b) {
      mvcMessaging.notify("model:ageCheck", 0, true);   
     }
     else if (loverAge > d) {
      mvcMessaging.notify("model:ageCheck", 1, true);
     }
     else {
      mvcMessaging.notify("model:ageCheck", 2, true);   
     }
     }
    }
